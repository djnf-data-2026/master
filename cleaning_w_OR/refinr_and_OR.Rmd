...
This notebook contains code and explanatory text from "Data Journalism with R and the Tidyverse," a free textbook used to teach UMD's JOUR 472/772 classes. The textbook can be found at https://thescoop.org/data_journalism_book/. This notebook is pulled from Derek Willis's Github repository for the textbook materials https://github.com/dwillis/datajournalismbook-maryland. The notebook was originally named pre_lab_03.Rmd, and can be found in pre_labs/pre_lab_03. All credit for the materials in this notebook belongs to its original authors.

## OpenRefine

In this session, we're going to familiarize ourselves with a powerful tool for cleaning and standardizing data — a program called OpenRefine.

Open Refine is free software. You should download and install it; the most recent version is 3.10.0 (https://openrefine.org/download)

We're going to start by learning how to use OpenRefine using code in R, with a package called Refinr. Refinr is great for quick things on smaller datasets that you can check to make sure it’s not up to any mischief.
